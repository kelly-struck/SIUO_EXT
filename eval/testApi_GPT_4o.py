...
for item in data[start_index:]:
    ...
    try:
        ...
        print(f"Question ID {item['question_id']} - Full API response: {completion}")
        if completion and hasattr(completion, 'choices') and completion.choices and len(completion.choices) > 0:
            if hasattr(completion.choices[0], 'message') and completion.choices[0].message:
                if hasattr(completion.choices[0].message, 'content') and completion.choices[0].message.content:
                    ...
                    error_msg = "The API returns an empty or non-existent message content."
                    print(f"Question ID {item['question_id']}: {error_msg}")
                    item['answer'] = f"Error: {error_msg}"
            else:
                error_msg = "The selected option returned by the API does not contain a message object."
                print(f"Question ID {item['question_id']}: {error_msg}")
                item['answer'] = f"Error: {error_msg}"
        else:
            # 检查是否有错误信息
            if hasattr(completion, 'error') and completion.error:
                error_msg = f"API returned an error: {completion.error}"
                print(f"Question ID {item['question_id']}: {error_msg}")
                item['answer'] = f"Error: {error_msg}"
            else:
                error_msg = "The API returned an invalid response structure."
                print(f"Question ID {item['question_id']}: {error_msg}")
                item['answer'] = f"Error: {error_msg}"
